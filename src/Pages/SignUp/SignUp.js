...
const SignUp = () => {
    ...
    const handleSignup = (event) => {
        ...
        createUser(email, password)
            ...
            .catch(error => {
                console.error(error);
                setError(error.message.slice(22, error.message.length - 2))
            })
        ...
        <div class="w-full max-w-sm p-6 m-auto mx-auto rounded-md shadow-md bg-purple-300  my-10">
            ...
            <form onSubmit={handleSignup} class="mt-6">
                <div>
                    ...
                    <input type="text" name='name' class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 dark:focus:border-blue-300 focus:ring-blue-300 focus:outline-none focus:ring focus:ring-opacity-40" required />
                </div>
                <div>
                    <label for="photoURL" class="block text-sm text-gray-800 dark:text-gray-200">photoURL</label>
                    <input type="text" name='photoURL' class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 dark:focus:border-blue-300 focus:ring-blue-300 focus:outline-none focus:ring focus:ring-opacity-40" />
                </div>
                <div>
                ...
                <div class="mt-4">
                    <div class="flex items-center justify-between">
                        ...
                        <Link href="#" class="text-xs text-gray-900  hover:underline">Forget Password?</Link>
                    </div>
                ...
                <small className='text-red-600 text-base my-2'>{error} </small>
            ...
            <p class="mt-8 text-xs font-light text-center text-gray-900"> Already have an account? <Link to='/login' class="font-medium text-gray-700 dark:text-gray-200 hover:underline">Sign In</Link></p>
        </div>
    );
};
